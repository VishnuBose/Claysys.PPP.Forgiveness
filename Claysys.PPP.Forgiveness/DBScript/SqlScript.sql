...
/****** Object:  StoredProcedure [dbo].[spi_ForgivenessMessage]    Script Date: 8/14/2020 4:27:28 AM ******/
...
/****** Object:  StoredProcedure [dbo].[Sps_ForgivenessApplicationSelect]    Script Date: 8/14/2020 4:22:00 AM ******/
...
BEGIN
	(
			SELECT TOP 50 LF.SBAPPPLoanNumber AS SBALoanNumber
				...
				,CASE 
					WHEN PLF.STATUS = 'APPROVED IN FULL'
						THEN 0
					WHEN PLF.STATUS = 'APPROVED IN PART'
						THEN 1
					WHEN PLF.STATUS = 'DENIED'
						THEN 2
					WHEN PLF.STATUS = 'DENIED WITHOUT PREJUDICE DUE TO PENDING SBA REVIEW'
						THEN 3
				...
				,CASE 
					...
					ELSE DG.Position
					END AS DemoPosition
				,DG.Gender AS DemoGender
				,DG.Ethnicity AS DemoEthinicity
				,DG.Race AS DemoRace
				,DG.Veteran AS DemoVeteran
			...
			WHERE (
					PLF.STATUS = 'APPROVED IN FULL'
					OR PLF.STATUS = 'APPROVED IN PART'
					OR PLF.STATUS = 'DENIED'
					OR PLF.STATUS = 'DENIED WITHOUT PREJUDICE DUE TO PENDING SBA REVIEW'
					)
	...
	(
		SELECT TOP 50 EZ.SBALoanNumber
			...
			,CASE 
				...
				ELSE Position
				END AS DemoPosition
			,Gender AS Demogender
			,Ethinicity AS DemoEthinicity
			,Race AS DemoRace
			,Veteran AS DemoVeteran
